...
function FC_fRedefineOpcoes(IDProduto,aDados,aDistintos,iCampo,ValorCampo){
  ...
  if(iCampo==0 && aDados.length>1){
   ...
   if(iCampo==0){
    ...
    document.getElementById('GrauProduto').innerHTML="<b>Op&ccedil;&atilde;o selecionada:</b> "+ValorCampo+"&nbsp;";	
   }
   else if(iCampo==2){
    ...
    if(oSubProd.Existe){
      ...
      if(!bSubProdSemPreco){
		...
		{		  
		  ...
		  document.getElementById('PrecoProdPrinc').innerHTML="<div class='center' style='width:300px'>de <strike>"+FormatPrice(oSubProd.PrecoOri,'R$')+"</strike> por <b>"+FormatPrice(oSubProd.PrecoNum,'R$')+"</b></div>";
         }
         else
		 {				
...
function FC_CompraSubProd(IDSubProd,Cor,Adicional1,Adicional2,Adicional3,AdicionalD1,AdicionalD2,AdicionalD3,IDProduto){
  document.getElementById('FC_oBotao_'+IDProduto).style.backgroundImage="url('"+sCaminhoImagens+"LoadingComprar.gif')"; //Altera botão Comprar para LOADING
  var aNomeRGB=Cor.split(SeparadorRGBCor);
  var sNomeCor=aNomeRGB[0];
  //var sParamsProd='&QTIncMult_'+IDSubProd+'=1';
  oQtd=document.getElementById("QtdProdMult");
  iQtd=oQtd.value;
  if(iQtd=='')iQtd=1;oQtd.value=iQtd;
  var sParamsProd='&QTIncMult_'+IDSubProd+'='+iQtd;
...
function FC_fInitProd(IDProduto){
  ...
  if(NivelSeguinte==-1)return void(0); 
   ...
   for(var i=0;i<FC_QtdDescritores;i++){
	...
	if (i==1) {titdesc="Selecione abaixo:";}
  ...
  document.write("<div class='gradeComprar os_relativo' id=EstTabBotComprar><div class='gradeComprarDados' onClick='alertaEscolha();' id=alertaEsconde></div><span id=CorProduto></span><span id=GrauProduto></span><span id=CurvaProduto></span><span id=MarcaProduto></span><span id=Campo1></span><span id=Campo2></span><span id=Campo3></span><br>Quantidade: <input type='text' value='1' id='QtdProdMult' onkeypress='return NumFilt(event)' class='InputText' style='width:25px;text-align:center;' maxlength=3><br><br><input type=button class=BotComprarGrid name=FC_oBotao_"+ IDProduto +" id=FC_oBotao_"+ IDProduto +" value='' style='z-index:1;cursor:pointer'><br><div id=alerta>ATEN&Ccedil;&Atilde;O: Selecione as op&ccedil;&otilde;es do produto acima.</div></div>");
