...
function FC_fRedefineOpcoes(IDProduto,aDados,aDistintos,iCampo,ValorCampo){
  ...
  for(var j=0;j<aDistintos[iCampo].length;j++){	  
    ...
    if(aDistintos[iCampo][j]!=ValorCampo){
      ...
      FC_Option.style.borderWidth='2px';
      FC_Option.style.borderStyle='solid';
      FC_Option.style.borderColor='#7C9923';
    ...
    if(oSubProd.Existe){
      ...
      if(!bSubProdSemPreco){
        ...
        if(oSubProd.iEstoque>0){
          ...
          oBotao.onclick=new Function('FC_CompraSubProd("'+ oSubProd.IDSubProduto +'","'+oSubProd.Cor+'","'+oSubProd.Adicional1+'","'+oSubProd.Adicional2+'","'+oSubProd.Adicional3+'","'+oSubProd.AdicionalD1+'","'+oSubProd.AdicionalD2+'","'+oSubProd.AdicionalD3+'");');	
        }
        else{
...
function FC_CompraSubProd(IDSubProd,Cor,Adicional1,Adicional2,Adicional3,AdicionalD1,AdicionalD2,AdicionalD3){
  ...
  if(sNomeCor!='')sURLCompra+='&Cor='+Encoder.htmlDecode(sNomeCor).replace(/\+/g,'%2B');
  if(Adicional1!='')sURLCompra+='&Adicional1='+Encoder.htmlDecode(Adicional1);
  if(Adicional2!='')sURLCompra+='&Adicional2='+Encoder.htmlDecode(Adicional2);
  if(Adicional3!='')sURLCompra+='&Adicional3='+Encoder.htmlDecode(Adicional3);
  if(AdicionalD1!='')sURLCompra+='&AdicionalD1='+Encoder.htmlDecode(AdicionalD1);
  if(AdicionalD2!='')sURLCompra+='&AdicionalD2='+Encoder.htmlDecode(AdicionalD2);
  if(AdicionalD3!='')sURLCompra+='&AdicionalD3='+Encoder.htmlDecode(AdicionalD3);  
...
function FC_fInitProd(IDProduto){
  ...
  document.write("<div class='gradeComprar'><div class='gradeComprarDados' onClick='alertaEscolha();' id=alertaEsconde></div><span id=CorProduto></span><span id=GrauProduto></span><span id=CurvaProduto></span><span id=MarcaProduto></span><span id=Campo1></span><span id=Campo2></span><span id=Campo3></span><br><input type=button class=BotComprarGrid name=FC_oBotao_"+ IDProduto +" id=FC_oBotao_"+ IDProduto +" onClick='' value='' style='z-index:1;'><br><div id=alerta>ATEN&Ccedil;&Atilde;O: Selecione as op&ccedil;&otilde;es do produto acima.</div></div>");
